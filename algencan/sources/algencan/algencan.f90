...
subroutine algencan(fsub,gsub,hsub,csub,jacsub,hcsub,fcsub,gjacsub, &
     ...
     eostain,efacc,eoacc,outputfnm,specfnm,nvparam,vparam,ar3_theta,&
     ar3_p,norig,xorig,lorig,uorig,m,lambda,equatn,linear,coded,    &
     checkder,fu,cunorm,snorm,nlnorm,geninfo,inform)
  ...
  use probgiven, only: iniprobgiven,firstde,seconde,truehpr,gjacpcoded

  implicit none

  ! SCALAR ARGUMENTS
  logical,      intent(in)    :: checkder
  integer,      intent(in)    :: hnnzmax,jcnnzmax,m,norig,nvparam,ar3_p
  integer,      intent(out)   :: geninfo,inform
  real(kind=8), intent(inout) :: cunorm,efacc,efstain,eoacc,eostain, &
                                 epsfeas,epsopt,fu,nlnorm,snorm,ar3_theta
  ...
  integer      :: alinfo,allocerr,iter,maxit,n,nwcalls,nwtotit, &
                  outiter,solinfo,totiter,msqcalls,msqtotit,i,best_outit, &
                  secoinfo
...
!!$     end if
  ...
  call allocatearrays(n,m,jcnnzmax,hnnzmax,allocerr)
  ...
  if ( .false. ) then
     ...
     call gencan(n,x,l,u,m,lambda,equatn,linear,rho,epsfeas,epsopt, &
          efstain,eostain,ar3_theta,ar3_p,maxit,iter,&
          f,nlnorm,cnorm,cunorm,geninfo,inform)
