...
subroutine gencan(n,x,l,u,m,lambda,equatn,linear,rho,epsfeas, &
     epsopt,efstain,eostain,ar3_theta,ar3_p, &
     maxit,iter,f,gpsupn,cnorm,cnormu,geninfo,inform)
  ...
  ! SCALAR ARGUMENTS
  integer,      intent(in)    :: m,maxit,n,ar3_p
  integer,      intent(inout) :: inform
  integer,      intent(out)   :: geninfo,iter
  real(kind=8), intent(in)    :: epsfeas,epsopt,efstain,eostain,ar3_theta
  ...
  if ( ( .not. ignoref .and. &
       gpsupn .le. min(epsopt,ar3_theta*xeucn**ar3_p) .and. iter > 0 ) .or. &
  ...
  if ( iprintinn .eq. 1 ) then
     ...
     write(* ,1060) iter,f,gpsupn,xsupn,min(epsopt,ar3_theta*xeucn**ar3_p),ssupn, &
                    nind,lvfit,innit,fcnt
     write(10,1060) iter,f,gpsupn,xsupn,min(epsopt,ar3_theta*xeucn**ar3_p),ssupn, &
                    nind,lvfit,innit,fcnt
 ...
 1040 format(/,5X,'It',9X,'function',2X,'gpsupn',3X,'xsupn',1X, &
               'epsilon',1X,'stesupn',3X,'nfree',3X,'SPGit',1X,'in-face',4X,'fcnt')
 1050 format(     I7,1X,1P,D16.8,2(1X,1P,D7.1))
 1060 format(     I7,1X,1P,D16.8,4(1X,1P,D7.1),4(1X,I7))
